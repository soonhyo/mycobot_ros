...
class HairSegmentationNode(object):
    def __init__(self):
        ...
        self.kernel_e = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (31,31))
    ...
    def process_image(self, msg):
        ...
        masked_img = np.zeros(img.shape[:2], dtype=np.uint8)
        
        img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        img_clahe = self.clahe.apply(img_gray)
        
        masked_img[np.where(hair_mask> 0)]= img_clahe[np.where(hair_mask>0)]
