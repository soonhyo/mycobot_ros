...
class OrientationNode(object):
    def __init__(self):
        ...
        # for si figure
        self.pub_clahe = rospy.Publisher("~output/clahe", Image, queue_size=1)
        self.pub_binary = rospy.Publisher("~output/binary", Image, queue_size=1)
        self.pub_coh = rospy.Publisher("~output/coh", Image, queue_size=1)
        self.pub_angle = rospy.Publisher("~output/angle", Image, queue_size=1)
        self.pub_disorder = rospy.Publisher("~output/disorder", Image, queue_size=1)
        ...
        self.max_iter_path = 3
        self.k = int(2*self.r*self.sc)
    ...
    def mask_refine_from_hsv(self,img):
    ...
    def process_image(self, msg, msg_mask):
        ...
        try:
            ...
            rospy.logerr(err)
            return
        if self.signal_loop == 0:
            signal_msg = rospy.wait_for_message("/signal_loop", Int8)
            self.signal_loop = signal_msg.data
        ...
        # _,th3 = cv2.threshold(img, 127, 255, cv2.THRESH_TRUNC | cv2.THRESH_OTSU)
        ...
        # _,th3 = cv2.threshold(img_c, 127, 255, cv2.THRESH_TRUNC | cv2.THRESH_OTSU)
        # img_c = th3
        ...
        cv2.putText(img_edge, '{:.2f} ms'.format(elapsed*1000), (10, 30), cv2.FONT_HERSHEY_SIMPLEX,1.0, (0, 255, 0), thickness=2)
        cv2.putText(dst, '{:.2f} ms'.format(elapsed*1000), (10, 30), cv2.FONT_HERSHEY_SIMPLEX,1.0, (0, 255, 0), thickness=2)
        ...
        msg_clahe =  self.bridge.cv2_to_imgmsg(img_clahe, encoding="mono8")
        msg_binary =  self.bridge.cv2_to_imgmsg(img_binary, encoding="mono8")
        msg_coh =  self.bridge.cv2_to_imgmsg(img_coh, encoding="mono8")
        msg_angle =  self.bridge.cv2_to_imgmsg(img_angle, encoding="bgr8")
        msg_disorder =  self.bridge.cv2_to_imgmsg(img_disorder, encoding="bgr8")
        ...
        # for si figure
        self.pub_clahe.publish(msg_clahe)
        self.pub_binary.publish(msg_binary)
        self.pub_coh.publish(msg_coh)
        self.pub_angle.publish(msg_angle)
        self.pub_disorder.publish(msg_disorder)
