...
class TrajectoryHair(object):
    ...
    def get_axis(self, pcd, pcd_path):
        ...
        result_y =[]
        result_x =[]
        print("pcd_path:", len(np.asarray(pcd_path.points)))
    ...
    def get_pose_array(self, positions, qtns):
        ...
        # pose_array.header.frame_id = "realsense_torso_color_optical_frame"
    ...
    def path_cb(self, msg_path):
        ...
        # points_msg = rospy.wait_for_message('/realsense_torso/depth_registered/points', PointCloud2)
        ...
        #normal vector
        downpcd = self.get_normal(downpcd, r= 3, n= 400, viz = False)

        #rnn
        downpcd, points_nst, idx = self.search_neighboring_points_radius(downpcd, pcd_path.points[int(len(pcd_path.points)/2)], 0.1)

        pcd_rnn = o3d.geometry.PointCloud()
        pcd_rnn.points = o3d.utility.Vector3dVector(points_nst)
        pcd_rnn.paint_uniform_color([0, 1, 0])

        rnn_plane_model, rnn_inliers, rnn_outliers = self.ransac_plane(pcd_rnn, viz=False)
        rnn_az = self.norm(rnn_plane_model[:3])
        print(rnn_az)
        # np.asarray(downpcd.normals)[idx[1:],:] = rnn_az
        np.asarray(downpcd.normals)[:,:] = rnn_az
        ...
        # print(list(zip(ax, ay, az)))
        R = np.array(list(zip(ax, ay, az))).transpose(0,2,1)
        q = quaternion.from_rotation_matrix(R)
        pose_array = self.get_pose_array(pcd_path.points, q)
